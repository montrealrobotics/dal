...
class LocalizationNode(object):
    ...
    def update_act_dist(self, ax):
        ...
        if self.obj_act_act == None :
            if self.action_from_policy is not -1:
                self.obj_act_act = ax.text(self.action_from_policy, y[self.action_from_policy], '*')
        else:
            if self.action_from_policy is not -1:
                self.obj_act_act.set_position((self.action_from_policy, y[self.action_from_policy]))
        ...
        for i_worker in range(min(self.args.n_workers, self.grid_dirs)):
            ...
            if len(my_dirs)>0:
                pro = multiprocessing.Process(target = self.get_gtl_cos_mp2,
                                          args = [my_dirs, scan_data, return_dict])
                procs.append(pro)
    ...
    def get_a_scan(self, x_real, y_real, offset=0, scan_step=1, noise=0, sigma=0):
    ...
    def get_a_scan_mp(self, range_place, return_dict, offset=0, scan_step=1, map_img=None, xlim=None, ylim=None):
    ...
    def get_synth_scan_mp(self, scans, map_img=None, xlim=None, ylim=None):
        ...
        for worker in range(min(self.args.n_workers, n_places)):
            ...
            kwargs = {'scan_step': self.args.pm_scan_step, 'map_img':map_img, 'xlim':xlim, 'ylim':ylim}
    ...
    def get_virtual_lidar(self, current_pose):
        ranges = self.get_a_scan(current_pose.x, current_pose.y, offset=current_pose.theta)
    ...
    def get_reward(self):
        ...
        if self.args.penalty_for_block != 0 and self.collision == True:
            self.reward_vector[0] -= self.args.penalty_for_block
            self.reward += -self.args.penalty_for_block
