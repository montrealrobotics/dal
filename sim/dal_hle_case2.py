...
class LocalizationNode:
    def __init__(self, args):
        ...
        self.obj_lik = None
        self.obj_lik_high = None
        self.obj_bel = None
        self.obj_bel_high = None

        self.obj_bel_dist = None
        self.obj_bel_dist_high = None        
        self.obj_gtl_dist = None
        self.obj_gtl_dist_high = None        
        self.obj_lik_dist = None
        self.obj_lik_dist_high = None
    ...
    def init_figure(self):
        ...
        if self.args.figure == True:# and self.obj_fig==None:
            ...
            self.gridspec = gridspec.GridSpec(3,6)
            ...
            self.ax_bel_high =  plt.subplot(self.gridspec[0,2])            

            self.ax_gtl =  plt.subplot(self.gridspec[1,1])
            self.ax_gtl_high =  plt.subplot(self.gridspec[1,2])            

            self.ax_lik =  plt.subplot(self.gridspec[2,1])
            self.ax_lik_high =  plt.subplot(self.gridspec[2,2])            


            self.ax_pbel =  plt.subplot(self.gridspec[0,3])
            self.ax_pbel_high =  plt.subplot(self.gridspec[0,4])

            self.ax_pgtl =  plt.subplot(self.gridspec[1,3])
            self.ax_pgtl_high =  plt.subplot(self.gridspec[1,4])
            
            self.ax_plik =  plt.subplot(self.gridspec[2,3])
            self.ax_plik_high =  plt.subplot(self.gridspec[2,4])

            self.ax_act = plt.subplot(self.gridspec[0,5])
            self.ax_rew = plt.subplot(self.gridspec[1,5])
            self.ax_err = plt.subplot(self.gridspec[2,5])
    ...
    def update_figure(self, newmap=False):
        ...
        ## BELIEF - HIGH ##
        ax=self.ax_bel_high
        self.update_belief_plot_high(ax)
        ...
        ## LIKELIHOOD HIGH ##
        ax=self.ax_lik_high
        self.update_likely_plot_high(ax)
        
        ax=self.ax_pbel 
        self.update_bel_dist(ax)
        ax=self.ax_pbel_high 
        self.update_bel_dist_high(ax)
        ax=self.ax_pgtl 
        ...
        ax=self.ax_plik_high
        self.update_lik_dist_high(ax)
    ...
    def update_likely_plot_high(self,ax):
        lik = self.likelihood_high.cpu().detach().numpy()
        lik, side = square_clock(lik, self.grid_dirs)
        if self.obj_lik_high == None:
            self.obj_lik_high = ax.imshow(lik,interpolation='nearest')
            ax.grid()
            ...
            ax.tick_params(axis='x', labelbottom='off')
            ax.tick_params(bottom="off", left="off")
            ax.set_title('Likelihood (HLE)')
        else:
            self.obj_lik_high.set_data(lik)
        self.obj_lik_high.set_norm(norm = cm.Normalize().autoscale(lik))
    ...
    def update_bel_dist_high(self,ax):
        y = (self.belief_high.cpu().detach().numpy().flatten())
        if self.obj_bel_dist_high == None:
            x = range(y.size)
            self.obj_bel_dist_high, = ax.plot(x,y,'.')
            self.obj_bel_max, = ax.plot(np.argmax(y), np.max(y), 'x', color='r', label='bel')
            ax.set_ylim([0, y.max()*2])
            # ax.set_ylabel('Belief')
            # ax.set_xlabel('Pose')
            ax.set_title("Belief (HLE)")
        else:
            self.obj_bel_dist_high.set_ydata(y)
            self.obj_bel_max.set_xdata(np.argmax(y))
            self.obj_bel_max.set_ydata(np.max(y))
            ax.set_ylim([0, y.max()*2])
    ...
    def update_lik_dist_high(self,ax):
        y = (self.likelihood_high.cpu().detach().numpy().flatten())
        if self.obj_lik_dist_high == None:
            x = range(y.size)
            self.obj_lik_dist_high, = ax.plot(x,y,'.')
            self.obj_lik_max_high, = ax.plot(np.argmax(y), np.max(y), 'rx')
            ax.set_ylim([0, y.max()*2])
            # ax.set_ylabel('Likelihood')
            # ax.set_xlabel('Pose')
            ax.set_title("Likelihood (HLE)")
        else:
            self.obj_lik_dist_high.set_ydata(y)
            self.obj_lik_max_high.set_ydata(np.max(y))
            self.obj_lik_max_high.set_xdata(np.argmax(y))
            ax.set_ylim([0, y.max()*2])
    ...
    def update_belief_plot_high(self,ax):
        bel = self.belief_high.cpu().detach().numpy()
        bel,side = square_clock(bel, self.grid_dirs)
        if self.obj_bel_high == None:
            self.obj_bel_high = ax.imshow(bel,interpolation='nearest')
            ax.grid()
            ticks = np.linspace(0,self.map_rows*side, side,endpoint=False)-0.5
            ax.set_yticks(ticks)
            ax.set_xticks(ticks)
            ...
            ax.tick_params(axis='x', labelbottom='off')
            ax.tick_params(bottom="off", left="off")
            ax.set_title('Belief Lev-1 (%.3f)'%self.belief_high.cpu().detach().numpy().max())
            
        else:
            self.obj_bel_high.set_data(bel)
            ax.set_title('Belief Lev-1 (%.3f)'%self.belief_high.cpu().detach().numpy().max())

        self.obj_bel_high.set_norm(norm = cm.Normalize().autoscale(bel))
    ...
    def compute_wasser_low(self):
        wasser = 0
        for i in range(self.grid_dirs):
            for j in range(self.grid_rows):
                for k in range(self.grid_cols):
                    wasser += self.manhattanAB((i,j,k), self.get_true_grid_low()) * self.belief[i,j,k]
        return wasser.item()
        
        
    def compute_wasser_high(self):
        wasser = 0
        for i in range(self.grid_dirs):
            for j in range(self.map_rows):
                for k in range(self.map_cols):
                    wasser += self.manhattanAB((i,j,k), self.get_true_grid_high()) * self.belief_high[i,j,k]
        return wasser.item()
    ...
    def report_status(self,end_episode=False):
        ...
        wasser_low = self.compute_wasser_low()
        wasser_high = self.compute_wasser_high()        
        if self.args.save:
            with open(self.log_filepath,'a') as flog:
                flog.write('%d %d %d %f %f %f %f %f %f %f %f %f %f %f %e %e %f %f %f %f\n'%(self.env_count, self.episode_count,self.step_count,
                                                                                            self.loss_ll0, self.loss_ll1, dist, wasser_low, wasser_high, reward,
        ...
        print('%d %d %d %f %f %f %f %f %f %f %f %f %f %f %e %e %f %f %f %f'%(self.env_count, self.episode_count,self.step_count,
                                                                             self.loss_ll0, self.loss_ll1, dist, wasser_low, wasser_high, reward,
                                                        self.loss_policy, self.loss_value, 
                                                        self.prob[0,0],self.prob[0,1],self.prob[0,2],
                                                        lr_rl,
    ...
    def get_true_grid_high(self):
        row=to_index(self.current_pose.x, self.map_rows, self.xlim)
        col=to_index(self.current_pose.y, self.map_cols, self.ylim)
        heading = self.current_pose.theta
        
        head = self.grid_dirs * wrap(heading + np.pi/self.grid_dirs) / 2.0 / np.pi
        head = int(self.true_grid.head % self.grid_dirs)
        return (head,row,col)

    def get_true_grid_low(self):
        row=to_index(self.current_pose.x, self.grid_rows, self.xlim)
        col=to_index(self.current_pose.y, self.grid_cols, self.ylim)
        heading = self.current_pose.theta
    ...
    def compute_gtl(self, ref_scans):
        if self.args.gtl_off == True:
            ...
            self.gt_likelihood = self.gt_likelihood * self.the_mask.cpu().detach().numpy()
    ...
    def compute_gtl_high(self, ref_scans):
        if self.args.gtl_off == True:
            ...
            if self.args.gtl_src == 'hd-corr':
                ...
                raise Exception('GTL source required: --gtl-src= [low-dim-map, high-dim-map]')
            self.gt_likelihood_high = self.gt_likelihood_high * self.the_mask_high.cpu().detach().numpy()
    ...
    def update_target_pose(self):
        ...
        if self.action_str == "turn_right":
            ...
            ## bring it back to the center of current belief_low:
            self.updat_believed_pose()
            self.goal_pose.x = self.believed_pose.x + math.cos(self.believed_pose.theta)*self.fwd_step_meters
            self.goal_pose.y = self.believed_pose.y + math.sin(self.believed_pose.theta)*self.fwd_step_meters
            self.goal_pose.theta = self.believed_pose.theta
    ...
    def transit_belief(self):
        ...
        self.belief_high = self.belief_high.cpu().detach().numpy()        
    ...
    def manhattanAB(self, guess, other):
        head,row,col = other
        e_dir = abs(guess[0]-head)
        e_dir = min(self.grid_dirs-e_dir, e_dir)
        return float(e_dir+abs(guess[1]-row)+abs(guess[2]-col))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-upm0', '--update-pm0-by', help="train PM with GTL,RL,both, none", choices = ['GTL','RL','BOTH','NONE'], default='NONE', type=str)
    parser.add_argument('-upm1', '--update-pm1-by', help="train PM with GTL,RL,both, none", choices = ['GTL','RL','BOTH','NONE'], default='NONE', type=str)
